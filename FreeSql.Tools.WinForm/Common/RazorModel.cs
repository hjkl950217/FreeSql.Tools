...
public class RazorModel
{
	...
	public string GetColumnDefaultValue(DbColumnInfo col, bool isInsertValueSql)
	{
		...
		if (cstype == typeof(DateTime) && DateTime.TryParse(defval, out var trydt)) return isInsertValueSql ? (fsql.Select<TestTb>() as Select0Provider)._commonUtils.FormatSql("{0}", defval) : $"DateTime.UnixEpoch";
